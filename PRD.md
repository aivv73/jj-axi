...
5. **Resolved progressive disclosure.** The sole routing artifact lives at `skills/jj-axi/SKILL.md`, where GitHub-based skill tooling can discover it directly. Its body carries the repository model, cross-command safety rules, and compact examples for high-frequency workflows so agents can act without speculative help probes. Version-matched `<command> --help` remains authoritative for exact flags, full command-specific semantics, limits, and recovery. The binary embeds the routing skill: bare invocation prints its body without frontmatter, `skill` prints it whole, and `skill --output` installs it, while `setup skill` remains an alias. `skill --full` prints the opt-in detailed reference from `docs/agent-reference.md`. jj-axi does not invoke npm, npx, or agent-specific installers.
...
- Agent discovery is skill-led with a bare-invocation view of the same content. jj-axi does not mutate agent configuration or install session hooks; see ADR 0004 and ADR 0007.
