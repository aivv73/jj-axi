...
| 8 | Content first | Applicable | Running `jj-axi` with no arguments returns the short bootstrap guide directly rather than generic help. `skill` routes to one command, each non-trivial `<command> --help` returns a version-matched mini execution guide, and `skill --full` exposes the broad reference only on demand. Fresh structured repository state remains explicit through `jj-axi inspect`. | Keep each disclosure layer focused and prevent command help from degrading into a flag-only listing. |
