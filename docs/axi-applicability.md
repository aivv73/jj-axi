...
| 10 | Consistent help | Applicable | Root and nested commands use one `--help` convention, unknown options fail consistently, and the canonical skill requires agents to read the installed command contract before execution. Non-trivial command help includes one workflow, example, and critical safety semantics. | Preserve semantic help tests as commands evolve. |
